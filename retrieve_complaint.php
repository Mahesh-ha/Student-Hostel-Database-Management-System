...
<article>
  <h1 style="padding-left: 290px;color: brown">COMPLAINTS</h1>
...
<tbody>
	<?php
		...
		if(mysqli_num_rows($findidres) == 1)
		{
			...
			$id = $ids['W_id'];
			$sql = "SELECT * FROM Student JOIN Complaints ON Student.Reg_no = Complaints.Reg_no WHERE Complaints.W_id = '$id'";
			...
			if(mysqli_num_rows($result)>0)
			{
				while($row = mysqli_fetch_assoc($result))
				{
					...
					<td><?php echo $row['Smobile1'];?></td>&nbsp;<td><?php echo $row['Subject'];?></td><td><?php echo $row['Complaint'];?></td></tr>
