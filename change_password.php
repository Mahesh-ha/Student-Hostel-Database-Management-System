...
<article>
  ...
  <form method="POST" action="">
    ...
    <div style="padding-left:408px">
      <input type="submit" name="submit" value="Save" id="hello" class="button">
    </div>
  </form>
  <script>
    var p;
...
if(isset($_POST['password'],$_POST['newpassword'],$_POST['cnewpassword']))
  {
    ...
    $query = mysqli_query($conn,"SELECT PWORD FROM Student WHERE PWORD = '$old'");
    $rows = mysqli_num_rows($query);
    if($rows == 1)
    {
        if($new == $cnew)
        {
            $sql = "UPDATE Student SET PWORD ='$new' WHERE PWORD = '$old'";  
